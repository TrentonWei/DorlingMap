...
namespace AlgEMLib
{
    ...
    public class AlgSnake : AlgEM
    {
        ...
        /// <summary>
        /// 邻近图的移位算法实现(两个Stage，其实就只有坐标更新那一块不一样)
        /// <param name="MaxForce">用于限制计算两个力时的最大力</param>
        /// <param name="MaxForce_2">用于限制全力力时的最大力</param> 
        /// </summary>
        public void DoDispacePG_CTP_Stage2(List<ProxiNode> FinalLocation, SMap sMap, double MinDis, double MaxForce, double MaxForce_2)
        {
            fV = new SnakesForceVector(this.ProxiGraph);
            //求吸引力-2014-3-20所用
            fV.OrigialProxiGraph = this.OriginalGraph;
            ...
            else
            {
                ...
                UpdataCoordsforPG_CTP2_Stage2();     //更新坐标
            }
            //输出受力点出的受力与当前移位值
            //O0999999999999999999999999999999999999999999999999999999999999999PO0Lthis.OutputDisplacementandForces(fV.ForceList);
            //输出各点的移位总和
        ...
        /// <summary>
        /// 更新坐标位置(更新了Map中要素的信息)
        /// Stage2的坐标更新，靠近控制点的点直接移动至其新位置
        /// </summary>
        private void UpdataCoordsforPG_CTP2_Stage2()
        {
            foreach (ProxiNode curNode in this.ProxiGraph.NodeList)
            {
                int index = curNode.ID;
                ...
                {
                    Force CacheForce=this.fV.GetForcebyIndex(curNode.ID);
