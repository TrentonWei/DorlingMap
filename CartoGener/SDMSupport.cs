...
namespace CartoGener
{
    ...
    class SDMSupport
    {
        ...
        /// <param name="TimeData_2"></param>
        /// <returns></returns>
        public double GetSim(Dictionary<IFeature, double> TimeData_1, Dictionary<IFeature, double> TimeData_2)
        {
            double Sim = 0;

            #region 欧式距离
            foreach (KeyValuePair<IFeature, double> Kv in TimeData_1)
            {
                Sim = Math.Abs(Kv.Value - TimeData_2[Kv.Key]) + Sim;
            }
            Sim = Sim / TimeData_1.Count;
        ...
        public double GetGroupSim(List<Dictionary<IPolygon, double>> TimeDataGroup_1, List<Dictionary<IPolygon, double>> TimeDataGroup_2,int GroupSimType)
        {
            ...
            if (SimList.Count > 0)
            {
                ...
                {
                    GroupSim = SimList.Min();
                }
                if (GroupSimType == 1)//返回类间最大距离
                {
                    GroupSim = SimList.Max();
                }
                if (GroupSimType == 2)//返回类间平均距离
                {
        ...
        /// GroupSimType=0 类间最短距离；GroupSimType=1 类间最大距离；GroupSimType=2 类间平均距离
        /// <returns></returns>
        public double GetGroupSim(List<Dictionary<IFeature, double>> TimeDataGroup_1, List<Dictionary<IFeature, double>> TimeDataGroup_2, int GroupSimType)
        {
            double GroupSim = 0; List<double> SimList = new List<double>();
        ...
        ///  GroupSimType=0 类间最短距离；GroupSimType=1 类间最大距离；GroupSimType=2 类间平均距离
        /// <returns></returns>
        public Dictionary<int, Dictionary<int, List<int>>> GetHierarchy_1(List<Dictionary<IFeature, double>> TimeSeriesData,int GroupSimType)
        {
            ...
            while (Level < TimeSeriesData.Count)
            {
                ...
                for (int i = 1; i < LevelValue.Count; i++)
                {
                    ...
                    List<Dictionary<IFeature, double>> TargetLevelTimeSeriesData = new List<Dictionary<IFeature, double>>();
                    ...
                    List<Dictionary<IFeature, double>> ComparableLevelTimeSeriesData = new List<Dictionary<IFeature, double>>();
        ...
        ///  GroupSimType=0 类间最短距离；GroupSimType=1 类间最大距离；GroupSimType=2 类间平均距离
        /// <returns></returns>
        public Dictionary<int, Dictionary<int, List<int>>> GetHierarchy_2(List<Dictionary<IPolygon, double>> TimeSeriesData, int GroupSimType)
        {
            Dictionary<int, Dictionary<int, List<int>>> Hierarchy = new Dictionary<int, Dictionary<int, List<int>>>();//int 表示层level层级；Dictionary<int, List<int>>表示对应层上的聚类结果
            ...
            #region 聚类结果
            int Level = 0;
            while (Level < TimeSeriesData.Count-1)
            {
                Dictionary<int, List<int>> LevelValue = Hierarchy[Level];//获取上一层级分类结果
                ...
                Level++;
                Hierarchy.Add(Level, CacheLevel_Dic);
            }
